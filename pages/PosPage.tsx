...
const PosPage: React.FC = () => {
  ...
  const processSaleBtnRef = useRef<HTMLButtonElement>(null);
  ...
  const dailyAuditData = useMemo(() => {
    ...
    const todaysSales = state.reports.filter(r => {
    ...
    todaysSales.forEach(sale => {
        ...
        if(sale.payment.currency === 'CUP') {
            systemTotals.expectedCUP += sale.payment.amountPaid - sale.payment.changeInCup;
        } else if (sale.payment.currency === 'MLC') {
            systemTotals.expectedMLC += sale.payment.amountPaid;
        } else if (sale.payment.currency === 'USD') {
            systemTotals.expectedUSD += sale.payment.amountPaid;
        }
    });
    
    const counted = {
        CUP: totalCountedCUP,
        MLC: parseFloat(countedMLC) || 0,
        USD: parseFloat(countedUSD) || 0,
    };
    
    const discrepancies = {
        diffCUP: counted.CUP - systemTotals.expectedCUP,
        diffMLC: counted.MLC - systemTotals.expectedMLC,
        diffUSD: counted.USD - systemTotals.expectedUSD,
    };
    ...
    setState(prev => {
        ...
        if (cupShortage < 0) {
            const newLogEntry: TransactionLogEntry = {
                ...
                investmentBalanceAfter: prev.investmentBalance, // Unchanged
        ...
        return { 
            ...
            investmentBalance: prev.investmentBalance // Balance is not affected by shortage directly
        };
    });
